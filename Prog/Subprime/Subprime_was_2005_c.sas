/**************************************************************************
 Program:  Subprime_was_2005_c.sas
 Library:  Hmda
 ...
 Description:  Create lists of top 10 high interest rate lenders for counties
...
data Loans_tbl (compress=no);
  ...
  set Hmda.Loans_&year (keep=ulender ucounty loantype action high_interest purpose amount);

  where loantype = "1" and action = "1" and 
        high_interest and purpose in ( "1", "3" );
...
proc summary data=Loans_tbl;
  ...
  output out=high_interest_totals (where=(_type_ in ( 2, 3 ) )) 
    sum= ;

run;

proc sort data=high_interest_totals;
  by ucounty _type_ descending amount;

proc print data=high_interest_totals;

run;

%let sheet = HighIntLenders;
